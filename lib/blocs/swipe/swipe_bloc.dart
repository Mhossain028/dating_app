...
import 'package:dating_app/models/model.dart';
...
class SwipeBloc extends Bloc<SwipeEvent, SwipeState> {
  ...
  Stream<SwipeState> _mapSwipeLeftToState(
    SwipeLeftEvent event, SwipeState state,
  )
  async*{
    if(state is SwipeLoaded){
      try{
        yield SwipeLoaded(users: state.users..remove(event.user));
      }
      catch(_){}
  ...
  Stream<SwipeState> _mapSwipeRightToState(
    SwipeRightEvent event, SwipeState state,
  )
  async*{
    if(state is SwipeLoaded){
      try{
